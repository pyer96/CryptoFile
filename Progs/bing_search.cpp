 ...
 * =+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+
 *
 * bingrequest.cpp - Simple cmd line application that makes an HTTP GET request
 *to bing searching and outputting the resulting HTML response body into a file.
 *
 * =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 ****/
  ...
  file_buffer<uint8_t>::open(outputFileName, std::ios::out)
      .then([=](streambuf<uint8_t> outFile) -> pplx::task<http_response> {
        *fileBuffer = outFile;
        ...
        // Encode the URI query since it could contain special characters like
        // spaces.
        http_client client(U("http://www.bing.com/"),
                           client_config_for_proxy());
        auto uri = uri_builder(U("/search"))
