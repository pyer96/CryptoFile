...
PromptShellForAuthorizationCode(OAuth2AuthorizationFlow *flow,
...
void DisplayList(const string &prefix, const string &title, const LIST &list) {
...
private:
  ...
  // populated.
  googleapis::util::Status GetEvent(const string &calendar_id,
                                    const string &event_id, Event *event);
...
util::Status CalendarSample::Startup(int argc, char *argv[]) {
  if ((argc < 2) || (argc > 3)) {
    string error = StrCat("Invalid Usage:\n", argv[0],
                          " <client_secrets_file> [<cacerts_path>]\n");
  ...
  if (status.ok()) {
    ...
    status =
        openssl_factory->SetPassphrase(flow_->client_spec().client_secret());
    if (!status.ok())
      return status;
...
void CalendarSample::ShowCalendars() {
  ...
  DisplayList<CalendarList, CalendarListEntry>("", "CalendarList",
                                               *calendar_list);
  std::cout << std::endl;
}
...
void CalendarSample::AddEvent(const string &calendar_id, Event *event) {
  std::unique_ptr<EventsResource_InsertMethod> method(
      service_->get_events().NewInsertMethod(&credential_, calendar_id,
                                             *event));
...
void CalendarSample::PageThroughAllEvents(const string &calendar_id,
                                          int num_per_page) {
...
util::Status CalendarSample::GetEvent(const string &calendar_id,
                                      const string &event_id, Event *event) {
  std::unique_ptr<EventsResource_GetMethod> method(
      service_->get_events().NewGetMethod(&credential_, calendar_id, event_id));

  return method->ExecuteAndParseResponse(event);
}

void CalendarSample::PatchEvent(const string &calendar_id, const Event &event) {
  std::unique_ptr<EventsResource_PatchMethod> method(
      service_->get_events().NewPatchMethod(&credential_, calendar_id,
                                            event.get_id(), event));
...
void CalendarSample::UpdateEvent(const string &calendar_id,
                                 const Event &event) {
  std::unique_ptr<EventsResource_UpdateMethod> method(
      service_->get_events().NewUpdateMethod(&credential_, calendar_id,
                                             event.get_id(), event));
...
void CalendarSample::Run() {
  ...
  std::cout << std::endl << kSampleStepPrefix << "Adding Calendar" << std::endl;
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    EventsResource_InsertMethod *method(service_->get_events().NewInsertMethod(
