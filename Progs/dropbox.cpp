...
Set the app key & secret strings below (i.e. s_dropbox_key, s_dropbox_secret,
etc.) To get key & secret, register an app in the corresponding service. Set
following entry in the hosts file: 127.0.0.1    testhost.local
*/
...
//
static const utility::string_t
    s_dropbox_key(U("548501522105-nlu66mpp4eqihdhsad4e6gk0l0sqfbn7.apps."
                    "googleusercontent.com"));
static const utility::string_t s_dropbox_secret(U("onrSqOp1gpwr64nn6hu_vfCQi"));
...
// This type of listener can be implemented in the back-end to capture and store
// tokens.
//
...
public:
  oauth2_code_listener(uri listen_uri, oauth2_config &config)
    ...
    m_listener->support([this](http::http_request request) -> void {
      std::cerr << "m_listener->support " << request.to_string() << '\n';
    ...
    if (is_enabled()) {
      ...
      if (!m_oauth2_config.token().is_valid_access_token()) {
        if (authorization_code_flow().get()) {
          std::cerr << "m_http_config.set_oauth2(m_oauth2_config)\n";
      ...
      ucout << "Skipped " << m_name.c_str()
            << " session sample because app key or secret is empty. Please see "
               "instructions."
            << std::endl;
    }
  }
...
public:
  dropbox_session_sample()
      : oauth2_session_sample(
            U("Drive"),
            "548501522105-nlu66mpp4eqihdhsad4e6gk0l0sqfbn7.apps."
            "googleusercontent.com",
    ...
    m_oauth2_config.set_scope("https://www.googleapis.com/auth/drive");
  }

protected:
  void run_internal() override {
    std::cerr << "run internal\n";
    /*
    http_client api(U("https://api.dropbox.com/1/"), m_http_config);
    ucout << "Requesting account information:" << std::endl;
    ucout << "Information: "
          << api.request(methods::GET, U("account/info"))
                 .get()
                 .extract_json()
                 .get()
          << std::endl;*/
